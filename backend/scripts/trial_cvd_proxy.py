"""
PLAN_MEJORA_MATEMATICA §38 (2026-08-20) — T1.2: diagnóstico de IC del proxy CVD.

Test pre-registrado en §38, escrito ANTES de correr. Familia signal_diagnosis,
n_trials=21 (20 consumidos + 1), umbral Bonferroni de la familia:
   |t| > z(1 - 0.05/42) = 3.038  (dos colas, n=21)

HIPÓTESIS: cvd_rolling z-rodante-causal (wind0w=20, z 100d min_periods=50)
predice fwd_return_20d con signo +1 (presión neta compradora acumulada
reciente, ponderada por volumen, con cierre al high → comprador).

MÉTODO (protocolo estándar de la familia, §25/§37):
   - Panel: universo 50, sin máscara de elegibilidad.
   - Feats: z rodante 100d por símbolo de cvd_rolling (principal) y
     cvd_slope_5bar (informativa). z ESTRICTAMENTE CAUSAL (sólo datos ≤ t).
...
CRITERIO (pre-registrado en §38, UN slot en el ledger):
   cvd_rolling_z: |t| > 3.038 con signo +1 en >= 2/3 ventanas -> CUMPLE.
   cvd_slope_5bar_z se reporta pero NO dispara veredicto.
...
# Umbral Bonferroni de la familia signal_diagnosis (n_trials=21, dos colas)
ALPHA_PER = 0.05 / (2 * 21)
THRESHOLD = float(stats.norm.ppf(1 - ALPHA_PER))  # 3.038
...
FACTOR_COLS = ["cvd_rolling", "cvd_slope_5bar"]
Z_SUFFIX = "_z"
MAIN_FEATURE = "cvd_rolling" + Z_SUFFIX
...
def build_panel() -> pd.DataFrame:
    ...
    for sym, df in price_data.items():
        ...
        cols = ["date", "symbol", "close"] + [c + Z_SUFFIX for c in FACTOR_COLS] + ["fwd_20"]
        ind.index.name = "date"
...
def main() -> int:
    out_path = os.path.join(
        "data", "cache",
        f"trial_cvd_proxy_{datetime.datetime.now():%Y%m%d_%H%M%S}.txt")
    ...
    out("=" * 78)
    out("PLAN_MEJORA_MATEMATICA §38 — T1.2: diagnóstico de IC del proxy CVD")
    out(f"Umbral familia signal_diagnosis (n=21, dos colas): |t| > {THRESHOLD:.3f}")
    ...
    feature_cols = [c + Z_SUFFIX for c in FACTOR_COLS]

    out("\nRank IC por ventana, por feature (signo esperado: +1)")
    header = f"{'feature':20s} {'ventana':7s} {'n_dias':>6s} {'mean_IC':>9s} {'SE_NW':>8s} {'t':>7s} {'L':>3s} {'sig+':>5s}"
    out(header)
    out("-" * len(header))
    for feat in feature_cols:
        ...
        for name, (start, end) in WINDOWS.items():
            ...
            out(f"{feat:20s} {name:7s} {res['n_days']:6d} {res['mean_ic']:+9.4f} "
                f"{res['se_nw']:8.4f} {res['t']:+7.2f} {L:3d} {sig:>5s}")

    # TOTAL (solo informativo, NO pre-registrado)
    out("\nTOTAL por feature (solo informativo, NO pre-registrado)")
    out(f"{'feature':20s} {'n_dias':>6s} {'mean_IC':>9s} {'SE_NW':>8s} {'t':>7s}")
    for feat in feature_cols:
        res, _ = window_stats(panel, feat)
        out(f"{feat:20s} {res['n_days']:6d} {res['mean_ic']:+9.4f} {res['se_nw']:8.4f} {res['t']:+7.2f}")
    ...
    cumple = n_sig >= 2
    out("\n" + "=" * 78)
    out(f"CRITERIO §38: {MAIN_FEATURE}: |t| > {THRESHOLD:.3f} con signo +1 en >= 2/3 ventanas")
