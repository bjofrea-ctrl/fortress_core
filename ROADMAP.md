...
| Gate | **A2 — contador de días limpios `backend/data/clean_days.json`** | 🟢 cerrado (2026-09-06, Kilo) | — | `backend/scripts/clean_days_counter.py` + invocación en `scripts/daily_signal_pipeline.sh` al final de la ventana 22:10 (best-effort: nunca rompe la corrida del pipeline). Evalúa las 3 condiciones del gate por día hábil con evidencia en el JSON: (a) rc=0 en las 3 ventanas programadas del `pipeline_diario.log`, (b) corrida del updater sin `PRECIOS: ERROR`, (c) `reconcile ... unexplained=0` del día (log diario o `pipeline_state.json`). Semántica de racha: día verificado-roto corta; `UNVERIFIED_C` puro ((a)+(b) OK, reconciler no corrió) no suma ni corta; días previos al deploy del reconciler (2026-09-04, f7281d0) marcados `UNVERIFIED_C` (decisión pre-declarada §A2). 19 tests `test_clean_days_counter.py` con formatos exactos de los logs reales (incluido el caso roto real del 05-09: rc=2 + `PRECIOS: ERROR - 84/102`). Validado contra producción: 17-26/08 pre-pipeline (a rota), 27/08-04/09 `UNVERIFIED_C`, sábado 05-09 no-hábil. **La racha oficial arranca el primer hábil con las 3 condiciones OK** (el 05-09 tiene hash drift sin declarar + updater con error de precios — no cuenta). Pendiente humano: cuando se declare el bump del hash drift vigente, A2 recomienza la racha automáticamente desde ese día. |
