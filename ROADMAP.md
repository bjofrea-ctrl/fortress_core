...
|---|---|---|---|---|
| Código **P0 nuevo** | Un archivo que no es ticker tumba los 170 símbolos del cache | 🟡 **ARREGLADO en rama aparte, pendiente merge** — `cline/fix-cache-calendar` @ `1d4b67d` | merge de esa rama + restart del server | `data/cache/fundamentals_panel.parquet` tiene índice `MultiIndex(date, symbol)`. `data_ingestion.py:89-91` arma la lista de símbolos con un glob ciego de `*.parquet` → `cache_integrity.py:587` hace `pd.DatetimeIndex(df.index)` y revienta → sube por `_integrity_hook` dentro de `download_data` → `_safe_download:384` lo atrapa **por ticker** y devuelve `None` → `load_universe` vacío → `Datos insuficientes: 0 días` (`regime_classifier.py:147`) → warmup del advisor muerto y el Detalle sin payload (medido: `/api/advisor/AAPL` no respondió en 10 min). Reproducido en solo lectura: CON ese archivo = TypeError en los 109 símbolos del log; SIN él = 5961 días OK. Fix: saltar y reportar el frame cuyo índice no sea de fechas (`cache_integrity.py:586-588`) o filtrar el glob a nombres tipo ticker. Doble fragilidad real: el guardián muere con input inesperado y el mensaje de error nombra a la víctima, no al culpable — así es como se leyó "cache contaminado". **Arreglado en rama aparte** (`1d4b67d`, sobre `main`): se saltea y reporta el frame cuyo índice no es de fechas; la lectura del parquet queda **fuera** del `try` porque `ArrowInvalid` es subclase de `ValueError` y perdonarla habría vuelto esto un fail-open peor. 3 tests nuevos, 82 passed en los 6 archivos vecinos, ruff limpio. A/B sin red sobre copia del cache (sin escribir en el real): **sin fix 0/30 símbolos cargados, con fix 30/30 (119.336 filas)**; sobre el cache completo, el calendario pasó de `TypeError` a 5961 días. No mergeado: el backend estaba fuera del pre-registro del slice UX (`PRE_REG_UX_SLICE2_G2_LIMPIEZA_20260914.md` §3) y la decisión de merge es de Boris. **Secuencia para desbloquear C6**: merge → restart del server → el warmup debería cargar. Aviso: el cache está al 2026-09-11, así que el primer `load_universe` llama a Yahoo ~2 veces por símbolo (medido: 63 llamadas para 30 símbolos con el downloader stubbeado) — decidir si eso está bien con la cuota actual |
...
- G2 · Widget TV externo + MarketOverview/LiveTicker solapados + 3 componentes muertos.
  ...
  causa probada y **fix hecho, en rama aparte** (`cline/fix-cache-calendar` @ `1d4b67d`; A/B sin red:
  0/30 símbolos cargados → 30/30). Falta merge + restart, y eso implica gastar cuota de Yahoo contra
  un cache que está al 2026-09-11. Detalle en el §4 corregido del pre-registro.
