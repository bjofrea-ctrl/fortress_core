...
| Código P2 | Código muerto adicional sin verificar (`ProbabilisticEngine` wrapper, `KellyPositionSizer` duplicado, `RiskParityAllocator`) | 🟢 verificado (2026-08-15, M8) | — | CONFIRMADO: `ProbabilisticEngine`+`KellyPositionSizer` solo los usa `scripts/test_probabilistic.py`; `RiskParityAllocator` solo `scripts/test_system.py` — sin tests ni uso en producción. PERO `probabilistic_engine.py` no es descartable: `backtest_engine.py:10-17` importa 6 clases en producción (BayesianOnlineUpdater, CopulaRiskAnalyzer, FatTailMonteCarlo, ProbabilityCalibrator, SignalQualityMetrics, WalkForwardValidator), `signal_engine.py:7` y `opportunities.py:23` también. Eliminar las 3 clases requiere decidir el destino de los 2 smoke scripts — decisión de Claude Code |
