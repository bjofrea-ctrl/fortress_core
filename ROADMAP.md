...
| Investigación | **PBO/CSCV momentum+RSI — pre-registro borrador** (Tarea PBO, OpenCode) | 🟡 **en curso — borrador sellado, en cola hasta liberar slot** (2026-08-22) | Kilo/Cline/OpenCode ocupados — no se ejecuta hasta liberar slot | **DRAFT sin corrida** (`PRE_REGISTRO_PBO_CSCV_MOM_RSI.md` en raíz, estado BORRADOR — no ejecutado, no se edita después). Hipótesis: baseline momentum+RSI (el único que sobrevivió — `signal_engine.py:85-90` w_mom 0.664/rsi 0.336, mom `pct_change(252)`, rsi 45-70→0.8, universo 50, ventana 2019-01-01→2026-08-04, W1/W2/W3, costos 0.0005, Sharpe W1 0.256/W2 −0.05/W3 0.53, DSR 0.07/0.02/0.17) seleccionado entre **~21 trials `signal_diagnosis`** no es artefacto de selección. Método: **CSCV Bailey et al.** — S=16 particiones cronológicas → C(16,8)=12 870 splits IS/OOS, ranking IS de las N=21 Sharpes OOS, **PBO = P(rank_OOS < mediana)**, histograma de logits. Criterio pre-registrado y explícito: **PBO < 0.10 = NO overfitting (CUMPLE)**, 0.10–0.20 zona gris, **≥0.20 = overfitting (NO_CUMPLE)** — PBO alto = backtest overfitting (elegir la mejor IS no generaliza). El script previo `pbo_cscv.py` con N=1 daba PBO=0.5 por construcción (antisimétrico) — no mide selección; este PBO con N=21 sí. Corrección: DSR/N_eff solo calibración (Tarea L BH→BY no se rehace por decisión del usuario); PBO ya captura los ~21 trials sin Bonferroni adicional. Artefacto esperado: `backend/data/cache/pbo_cscv_mom_rsi_<ts>.txt/.json`; script propuesto: `backend/scripts/pbo_cscv_mom_rsi.py`; ledger: familia `signal_diagnosis` 21→22 (`n_trials_consumidos=1`, `umbral="PBO<0.10"`). Riesgos declarados: heterogeneidad de las 21, autocorrelación/overlap 20d, N splits/tamaño mínimo IS/OOS. Checklist de no-ejecución sellado en el doc (§9). Próximo paso: implementar y correr en slot libre **sin tocar el criterio de §4**. |
