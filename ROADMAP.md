...
| Investigación | **Híbrido objetivo × subjetivo — handoff a OpenCode para definir el pre-registro** (2026-09-25, Cline → OpenCode vía Herdr) | 🔵 EN CURSO (OpenCode `working` al cierre de esta sesión) — correr está bloqueado por el gate | OpenCode resuelve D1-D7 y escribe el pre-registro; **correr SOLO post-gate (2026-12-01)** | Doc: `HANDOFF_HIBRIDO_OBJETIVO_SUBJETIVO.md` (commit `cb13ce3`). Tesis de Boris: la objetiva dice si hay edge, la subjetiva/acción si ya lo compraron → coinciden = INVERTIR con peso, no coinciden = VIGILAR con razón. Verificado: **la abstención con razón YA está implementada** (`decision.py::_state_rule`, con razón explícita por caso); la pata subjetiva está **desconectada en ambos extremos** (A9 apaga el LLM + `professor_memory.json` vacío, D4). Huecos medidos: **COT truncado en 2019** (`market_sentiment.py:93`) y cache stale al 04-08; `thermometer.py` (F&G) **nunca se congeló**; Finnhub nunca produjo datos; noticias = 0 módulos. Trampa citada como criterio: **IC mejor ≠ PnL** (ridge purgado, trial #13). Entregables esperados de OpenCode: `ANALISIS_OPENCODE_HIBRIDO.md` (decisiones + correcciones con `file:line`) y `PRE_REGISTRO_HIBRIDO_OBJETIVO_X_SUBJETIVO.md` |
| Investigación | **Auditoría de código de TradingAgents v0.5.1 (TauricResearch) + comparación con fortress** (2026-09-25, Cline) | 🟢 cerrada (análisis) — 5 patrones adoptables, C1-C3 sin riesgo para el gate | Decisión de Boris sobre ejecutar C1-C3 | Doc completo: `ANALISIS_TRADINGAGENTS_VS_FORTRESS.md`. Verificado clonando el repo (commit `35543d0`, v0.5.1): licencia **Apache-2.0, no MIT**; cero Sharpe/DSR/PBO/CV/Newey-West/Bonferroni/costos (`grep` = 0 hits) y su "backtest" **no simula cartera por diseño declarado**. Superior en ingeniería LLM: grafo LangGraph con checkpoint/resume, salida estructurada pydantic con reparación de campos, 16 proveedores con retry/backoff, guardas point-in-time centralizadas (`as_of`/`coverage_gap`/`withhold_live_profile`). Nosotros superiores en aparato de validación (DSR/PBO/trial_registry/MDE/conformal), ejecución (lag=1, costos medidos Alpaca, barrier labeling), integridad de datos, loop de ejecución con contabilidad, producto y test (1.017 funciones vs sin métrica equivalente). Adoptables: **C1** retry/backoff en `NvidiaNIMClient.generate()` (hoy 429 → warning + `None`), **C2** `PROVIDER_REGISTRY` en vez de slugs hardcodeados con routing por prefijo, **C3** look-ahead latente en `RAGMemorySystem.retrieve_agent_memory` (recupera lecciones sin filtro temporal — inactivo porque A9 tiene la capa apagada). C4/C5 (salida estructurada real, resume por paso) quedan para cuando la gobernanza se re-encienda |
...
| Motor | **A4 — hash-guard: drift de `backtest_engine.py` por A6 declarado y re-verify limpio** (Cline, 2026-09-03) | 🟢 cerrado (2026-09-03) | — | `motor_manifest verify` detectó drift real (rc=2) causado por A6 y no declarado al commitear. Antes de bump se verificó contra el diff que el cambio es 100% A6 (n_trials del ledger + trazabilidad; no toca señal, riesgo ni rebalanceo) → `bump --note` con la razón. `verify` → OK 7 módulos, rc=0. El reinicio del contador ya tiene dónde vivir: A2 cerrado 2026-09-06 (ver fila anterior). |
| Gate | **A2 — contador de días limpios `backend/data/clean_days.json`** | 🟢 cerrado (2026-09-06, Kilo) · **consolidado 3 implementaciones + reconciler DIARIO (tarde 06-09)** | — | Tres implementaciones nacieron en paralelo (Kilo 65c5d09, Cline 55606a3 + una sin commitear del repo real); auditoría cruzada contra logs de producción encontró bugs complementarios: la de Cline parseaba (a) con un regex que nunca matchea el formato real del log (timestamp con T inexistente) y relajó a "≥1 corrida" violando la definición congelada; la de Kilo perdía el `PRECIOS: ERROR` de corridas de mediodía (parseo por "Corrida <ts>" en vez de bloques `[ts] inicio`…`fin` — caso real del 02-09); el test de regresión A1 contaminaba el `pipeline_diario.log` real con líneas reconcile falsas en cada pytest. **Consolidación final (worktree test-kilo-orca)**: (a) las 3 ventanas rc=0 con pares start/end reales; (b) bloques inicio/fin del updater con multi-corrida/día; (c) última línea del día + state.json como fuente. **Reconciler DIARIO aprobado por Boris (06-09)**: `--reconcile` en la corrida 22:10 vía `_run_reconcile` compartido (decide queda intacto) — con decide mensual la (c) era verificable ~1 día/mes y la racha de 60 matemáticamente imposible. Semántica de racha ininterrumpida (misma que C1): TODO weekday desde el gate 02-09 es evaluable (updater muerto = día roto, no invisible); tras activarse la cadencia diaria (auto-calibrada: primer par de weekdays adyacentes con líneas reconcile), un weekday sin línea propia es `MISSING_AFTER_DAILY` = fallo verificado. `append_diario_log` resuelve su default EN runtime (monkeypatcheable — fin de la contaminación de tests al log real). Validado contra producción: 02-09 b rota con el ERROR del mediodía ahora capturado; 03/04-09 UNVERIFIED_C puro; racha oficial arranca 07-09 con reconciler diario. 24 tests counter + 4 reconciler-en-health + 121 suite afectada verde. |
| Datos | **Cache de precios contaminado + daemon API en loop patológico (Kilo, 2026-09-14)** | 🔴 abierto, motor BLOQUEADO hasta resolución | Evidencia: 28 grupos/66 tickers con colas idénticas (SPY=AAPL=TSLA=510.37 vs 764.29/332.27/365.44 live; vols 13x); ACN/PEP comparten 414 filas (divergen 2025-01-16); vs espejo Sep-2 el 100% de filas comunes difiere; VIX cache 96 vs 18.02 live; 108 parquets reescritos 08:23-08:32 en tandas. Daemon API al 82% CPU sostenido desde jueves: ciclos warmup 862s/1258s + hits 0.0s back-to-back (loop sin piso de main ante rebuild>intervalo), 453 líneas en api_server.log, cache_date null en vivo. Escritor exacto de tripletes pendiente de identificar (descartados: hook solo memoiza, tests usan tmp, updater corre 22:00). Candidatos a ordenar (Boris): (a) merge fix-forward con piso 60s (`fix/warmup-criterio2-20260911`, frena el spin); (b) recuperación cache (espejo Sep-2 + re-descarga limpia tras identificar escritor); (c) no correr decisiones del motor hasta entonces. Detalle en SESSION_LOG 2026-09-14 + ANALISIS_MIEDO_20260914.md. || Perf | **FIX criterio 2 warmup — ventana fría post-ciclo lento + `cache_date` null** (hallazgo OpenCode 2026-09-10 sobre `6f0c123` ya en main) | 🟢 mergeado a main 2026-09-14 (orden Boris) | — | **Bug 1 (criterio 2)**: `warmup_advisor_loop` dormía `interval_s` DESDE EL FIN del ciclo. Con rebuild real 531s > TTL 300s, el próximo re-warmup caía a gen+531+280 = 3.6min DESPUÉS del vencimiento → todo request de esa franja pagaba frío. El test del criterio 2 no lo veía: ciclos stubbeados instantáneos → intervalo fijo siempre despertaba antes del vencimiento. **Fix**: el loop ancla cada ciclo al VENCIMIENTO del cache (gen+TTL); `WARMUP_INTERVAL_SECONDS` pasa de "período" a **piso de reintento** 60s (evita spin de delay-0 en la patología pre-declarada rebuild>TTL; con cache fresco el ciclo es hit de ~ms, costo acotado). Nota de diseño: apuntar a gen+280 era un hit inútil — `_get_context` respeta el TTL, un ciclo que despierta con cache fresco no refresca nada. **Bug 2 (menor)**: `cache_date` null en log `advisor_warmup_complete`: `_cache_date()` pedía `columns=["Close"]` pero el cache vivo usa minúsculas (`close`) → `ArrowInvalid` tragado por el `except` → None siempre. Verificado contra parquets reales del worktree (SPY/AAPL → 2026-09-09). Fix: `columns=[]` (solo índice, no lee datos). Tests: +2 con FakeClock que reproducen el gap con ciclo lento (rebuild N+1 == gen+TTL exacto, cero ventana) y el piso anti-spin; +1 `_cache_date` con columnas lowercase; el regression guard de criterio 2 se hizo determinista (FakeClock) tras flakear en la suite completa (TTL=2s real + scan de ~110 parquets bajo carga). **Suite completa 852 tests (excluye 1 huérfano A6 untracked de otro frente, import roto preexistente): 830 passed, 5 skipped, 16 FAILED preexistentes verificados con stash (paper_trading 10 + predict_cache 3 + intraday_collector 2 + config_registry 1 — venv: TestClient starlette/httpx roto y AttributeError alpaca, documentados desde el commit original), 0 atribuibles al fix. Falsación: tests nuevos fallan con el loop viejo y con `columns=["Close"]` viejo. || Docs | **`PLAN_REMEDIO_BRECHAS_20260903.md` cita una "Regla 0 del ROADMAP" inexistente** (líneas 67, 68, 88) | 🔴 sin corregir en el plan (es ticket de Boris) | Corregirla a "Regla 1 de ONBOARDING.md" o el próximo agente vuelve a copiarla | Es el origen del error de cita que A7 arrastró; ya corregido en código, mensajes y tests. El ROADMAP no tiene reglas numeradas. |
| Datos | **Backoff/retry SEC en `fetch_edgar_universe_facts.py` — ola `ingestion_returned_none` en el screen (Cline, 2026-09-09)** | 🟢 cerrado (2026-09-09) | — | Reporte de Boris: tras el merge EDGAR, `run_fundamentals_screen` servía AAPL/MSFT/GOOGL/AMZN/NVDA (0 FMP) pero META+ caían a `reason=ingestion_returned_none` en bloque. **Diagnóstico (verificado, no hipótesis)**: el screen NO toca SEC — lee companyfacts locales (`load_edgar_companyfacts`); el cache `data/cache/edgar/` está gitignored, así que en caja fresca solo estaban los 5 del script viejo `fetch_edgar_fundamentals.py`; los 43 restantes caían a FMP y ese burst de 215 calls rebotaba en rate limit → ingesta None. La causa raíz de los companyfacts incompletos es el **throttling de SEC en ráfagas de 48** (mismo patrón ACN del commit 9062307), que el fetcher anterior tragaba como `fail` sin reintentar. **Fix**: `fetch()` con backoff exponencial + jitter (tope `SEC_MAX_BACKOFF`), respeto de `Retry-After`, reintento 429/503/5xx/URLError/timeout, escritura atómica `.part`+`os.replace`, gzip por magic header, `load_cik_map` reintentable, skip por tamaño 100KB→1KB, todo ajustable por env (`SEC_PACE_SECONDS/SEC_MAX_RETRIES/SEC_BASE_BACKOFF/SEC_MAX_BACKOFF/SEC_EDGAR_CACHE_DIR`) y `rc!=1` si algo falla (cron lo detecta). **Verificado**: burst completo 48/48 `fail=0` en cache temporal; ingesta caja-fresca → **47 `edgar_primary` + 0 llamadas FMP**, único `None` = XOM (límite 10-K documentado, no throttling); screen real sobre universo 50 → `completed=47 failed=1(XOM) calls_used=0`. Tests offline +4 (`tests/test_edgar_fundamentals.py`, 12 passed). Commit `a5c7415` en `edgar-fundamentals-adapter`, **SIN push/merge**. Pendiente operativo menor: `.gitignore` no cubre `backend/data/cache_fundamentals_ingestion/` (quedo untracked a mano).
| UX | **Slice 1 — dashboard como panel de control (pre-registro `PRE_REG_UX_DASHBOARD_CONTROL_PANEL_20260912.md`)** | 🟡 código cerrado en rama `ux/dashboard-control-panel`, SIN merge (espera OK de Boris) | — | Los 4 ítems del pre-registro commiteados: `efbfdd4` vintage derivado del JSON, `15b2b62` caveat investigación en KPICards, `2e6a258` factors del Score en MesaView, `63ad36c` estados vacíos RiskPanel, `38f49d3` estados vacíos MesaPage (universo vacío / cero INVERTIR = régimen adverso, la abstención es señal), `7c8ff29` definiciones inline (Win prob / Proyección §29 / Dist EMA / Stop / Target / Δ / gates / M2, cada título citado a `file:line` del motor) + leyenda de gates visible sin hover. Verificado CORRIDO 14-09: **vitest suite completa 66 passed (11 archivos)**, `./node_modules/.bin/tsc --noEmit` exit 0, `pytest test_backtest_api.py` 10 passed, `npm run build` exit 0 (909 módulos, 14.85s). **C4 (verificación visual en navegador) sigue ABIERTO** — no se afirma cerrado sin verlo. Gobernanza-500: rama aparte `fix/governance-500-nonfinite` validada (6 passed), worktree temporal `/tmp/gov500wt` limpiado. Superficie real vs `main` medida con `git diff --stat`: main YA tiene los ítems 1-3 en contenido; el delta vivo son MesaPage+tests, los tooltips y la documentación. |
