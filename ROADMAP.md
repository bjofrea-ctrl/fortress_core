...
| Instrumento | M4 — Costos medidos (Alpaca paper) | 🟡 hecho el caño, medición viva en curso (2026-08-17) | Apertura de mercado (lunes 2026-08-18 09:30 ET) | `app/core/execution_costs.py` (13 tests). Credenciales PAPER conectadas (cuenta `PA3QUWEX1XBJ`, ACTIVE, $25k sintéticos) vía `backend/.env` (gitignored). Smoke OK; fuera de rueda las market orders dan 422 (solo DAY/GTC limits para extended hours) → runner `scripts/run_costs_at_open.py` lanzado en background (PID 17770, log `data/cache/m4_runner.log`), corre buy+sell del universo al open. Pendiente: verificar los artefactos `measure_execution_costs_*.txt` + DB al completar y registrar el número de costo real medido |
